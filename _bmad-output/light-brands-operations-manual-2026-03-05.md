...
```mermaid
graph TD
    REV["REVENUE COMES IN"]
    OP["Pay operating expenses<br/>($50K/mo including salaries)"]
    LOAN["Repay Nicholas's $15K loan<br/>(per Agreement Section 2.4-2.5)"]
    RES{"Reserve at<br/>target level?"}
    FILL["Fund reserve until<br/>$600K-$1.2M reached"]
    DIST["Distributable surplus<br/>available for bonuses<br/>and distributions"]
    ...
    style RES fill:#b5838d,color:#fff
    style FILL fill:#0f3460,color:#fff
...
```mermaid
graph TD
    REV["Total Revenue Collected<br/>(in the quarter)"]
    OP["Minus: Operating Expenses<br/>($150K/quarter at $50K/mo)"]
    RES["Minus: Reserve Contribution<br/>(if reserve below target)"]
    POOL["= DISTRIBUTABLE SURPLUS"]
    BONUS["Bonus Pool<br/>(percentage of surplus)"]
    RETAIN["Retained Earnings<br/>(reinvestment, growth capital)"]
    ...
    style BONUS fill:#0f3460,color:#fff
```
