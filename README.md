...
graph TB
    ...
    subgraph Layer1["Layer 1: Operational Modules"]
        ...
        InvestOS["<b>InvestOS</b><br/>Capital Formation<br/>10 agents · 28 workflows"]
        AmplifyOS["<b>AmplifyOS</b><br/>Investor Comms<br/>10 agents · 24 workflows"]
        GrowthOS["<b>GrowthOS</b><br/>Growth Engine<br/>18 agents · 35 workflows"]
    end

    QI --> SoulOS --> CORE --> Layer1

    style QI fill:#4a1a8a,color:#fff,stroke:#7c3aed
    style SoulOS fill:#1a4a4a,color:#fff,stroke:#2dd4bf
    style CORE fill:#1a3a5c,color:#fff,stroke:#3b82f6
    style BuildOS fill:#1c4a1c,color:#fff,stroke:#22c55e
    style InvestOS fill:#4a3a1a,color:#fff,stroke:#f59e0b
    style AmplifyOS fill:#4a1a2a,color:#fff,stroke:#ef4444
    style GrowthOS fill:#1a2a4a,color:#fff,stroke:#6366f1
```
...
flowchart TD
    ...
    style CL fill:#1a3a2a,color:#fff
...
```mermaid
flowchart TD
    A["User Request"] --> B["QI Moral Gateway"]
    B -->|"enriched + ethics-checked"| C["Help Router"]
    C -->|"detects module + phase"| D["Agent Persona Loaded"]
    D --> E["Workflow Executed"]
    E --> F["Artifacts Produced"]
    F --> G["QI Post-Execution Capture"]
    G -->|"episode + lessons"| H[("Intuition Memory")]
    H -.->|"informs future decisions"| B

    style B fill:#4a1a8a,color:#fff,stroke:#7c3aed
    style G fill:#4a1a8a,color:#fff,stroke:#7c3aed
    style H fill:#1a4a4a,color:#fff,stroke:#2dd4bf
```
...
flowchart LR
    subgraph PRE["Pre-Execution"]
        M1["Detect Significance"] --> M2["Boundary Scan<br/><i>7 Divine Morals</i>"]
        M2 --> M3["Value Alignment<br/><i>7 Divine Values</i>"]
        M3 --> M4["Intuition Retrieval"]
        M4 --> M5["Route with Enrichment"]
    end
    ...
    subgraph POST["Post-Execution"]
        P1["Create Episode"] --> P2["Score AIQ<br/><i>6 cognitive dims</i>"]
        P2 --> P3["Score MIQ<br/><i>6 moral dims</i>"]
        P3 --> P4["Calculate TIS"]
        P4 --> P5["Extract Lessons"]
    end

    PRE --> EXEC --> POST

    style PRE fill:#1a2a3a,color:#fff
    style EXEC fill:#1c4a1c,color:#fff
